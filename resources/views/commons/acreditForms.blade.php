 ...
 */
?>
<form id="selectionForm" action="{{ route($vista.'List') }}" method="post">
    @csrf
...
{{--
<form id="showForm" action="{{ route($vista.'Show') }}" method="post">
    @csrf
    <input type="hidden" id="show_id" name="show_id" value="">
</form>--}}

<form id="editForm" action="{{ route($vista.'Edit') }}" method="post">
    @csrf
